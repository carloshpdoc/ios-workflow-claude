...
**CRITICAL: All verification MUST use a controlled A/B test.** Comparing benchmarks from different workspace generations, different build-graph states, or different times of day produces unreliable results. The A/B protocol below eliminates these variables.

#### Controlled A/B Test Protocol

Both the baseline and optimized benchmarks must be run back-to-back in a single session using identical methodology:

**Step A -- Baseline measurement:**
1. Revert project files to the pre-change state (e.g., `git checkout Project.swift`).
2. Regenerate the workspace (e.g., `tuist generate --no-open`) so the baseline uses a freshly generated project — identical to what the optimized run will use.
3. Run **2 warmup builds** (`xcodebuild build`, discard results) to settle Xcode's build graph and eliminate first-run artifacts like script-phase invalidation.
4. Benchmark with the standard script (5 runs, `--skip-warmup --no-cached-clean`).

**Step B -- Optimized measurement:**
5. Apply the approved changes to project files.
6. Regenerate the workspace again (same command as step 2).
7. Run **2 warmup builds** to settle the build graph.
8. Benchmark with the same script and flags as step 4.

**Step C -- Compare:**
9. Compare median wall-clock times from both artifacts. Only claim an improvement if the optimized median falls **outside** the baseline's min-max range.

This protocol ensures both runs share the same environmental conditions (machine load, thermal state, OS caches) and the same workspace-generation method, making the comparison fair.

#### Execution steps

10. Read `.build-benchmark/optimization-plan.md` and identify the approved items from the approval checklist.
11. Run the controlled A/B test (steps A through C above). The baseline benchmark must be collected **in the same session** as the optimized benchmark — do not reuse stale baseline data from earlier sessions.
12. Hand off implementation to `xcode-build-fixer` with the approved plan. The fixer applies each approved change and verifies compilation.
13. Append verification results to the optimization plan: post-change medians, absolute and percentage deltas, and confidence notes.
14. Package results: rename log files descriptively, zip into a single archive for documentation (e.g., `ab-test-YYYY-MM-DD/baseline/` and `ab-test-YYYY-MM-DD/optimized/`).
15. Report before and after results, plus any remaining follow-up opportunities.
...
- a link to the zipped A/B test archive containing all raw logs and JSON artifacts

### Stakeholder-Ready Report

Generate a standalone markdown report at `.build-benchmark/xcode-build-optimization-stakeholder-report.md` suitable for import into Notion or Confluence. Include:

- Results summary table (before/after/saved/improvement %)
- Time savings projection table (per day, week, month, quarter) based on estimated builds per day and team size
- Per-run wall-clock tables for clean and incremental builds
- Compiler workload breakdown (top categories, before/after)
- Changes made table (setting, config, before, after, purpose)
- Methodology section describing the A/B protocol
- Next opportunities table
...
```bash
python3 .claude/skills/scripts/diagnose_compilation.py \
  --project App.xcodeproj \
  --scheme MyApp \
  --configuration Debug \
  --destination "platform=iOS Simulator,name=iPhone 16" \
...
```bash
python3 .claude/skills/scripts/generate_optimization_report.py \
